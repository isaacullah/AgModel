...
This is an agent-based model of a potential scenario for the forager-farmer transistion. It is parameterized  for a millet/deer ecosystem (east Asia), but could apply to any hunting/seed gathering system if parameterized accordingly. Rather than use an existing ABM or other modeling framework, I have written it in pure Python (with the hope of better integration to scientific Python and other open-science movements).

The model consists of a single software agent that, conceptually, can be thought of as a single hunter-gather community (here, thought of as a co-residential group that shares in subsistence activities and decision making). The model assumes a logistic, central-place foraging strategy in a fixed territory (although residential mobility can be partially modeled by increasing the number of "migrating deer" per year). The territory has a fixed number of millet patches, and a starting number of deer. The simulation takes place at an annual temporal resoultion, although there are many foraging decisions that will be made per year. Each new year, the foraging agent must make a series of optimal foraging decisions based on its knowledge of the availability the two resources in the model: millet and deer (other resources are ignored, and their kcal intake should not account for the total number of kcals that the foraging agent uses to calculate its decisions). The agent proceeds to balance the net benefits of the chance of finding, processing, and consuming a deer, versus that of finding a millet patch, and processing and consuming that millet. These decisions continue until the annual kcal target is reached. If the agent consumes all available resources in a given year, it may "starve". Starvation will affect birth and death rates, as will foraging success. The initial birth and death rates of humans will thus change over time, and population will increase or decrease according to a probablistic function (perturbed by some stochasticity). Deer populations recover/grow according to a similar birth/death rate ratio. If agents consume millet, the selection preferences will drive evolutionary processess within the millet population (selection rate is set at the start of the simulation). Two parameters are selected for: morphology (grain size/easy of harvesting), and patch density (reseeding density). These rates will increase in patches that are under consistent use, but will begin to decrease according to a diffusion rate, and the proportion of utilized to unutilized millet patches. Morphologically wild millet produces fewer kcal per seed, and has higher handling times. Thus, as millet is more frequently used, it's desirable traights are enhanced, and it becomes more profitable. Millet patches are acessed in the same order every year. A small amount of deer may "migrate" into the territory each year. This prevents deer populations from complete decimation, but also may be used to model increased distances of logistic mobility (or, perhaps, even residential mobility within a larger territory). Note that while the model is not spatially explicit, it does assume some spatiality of resources (covered by search times).
