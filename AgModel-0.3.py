...
class Settings(eg.EgStore): #set up an Egstore class to store model settings
...
if __name__ == "__main__":
    ...
    for year in range(1,years+1):        #this is the outer loop, that does things at an annual resolution, counting the years down for the simulation
        ...
        while kcalneed > 0:        #this is the inner loop, doing foraging within the year, until kcal need is satisfied
            ...
            #print deerscore, milletscore, kcalneed
            if deerscore >= milletscore:        #check to see whether the band should eat deer or millet at this moment
                ...
                if htimebudget <= 0:
                    print "Ran out of hunting time this year, hopefully there is gathering time left"
                    deerscore = 0
                    pass
                if deer_now <= 0: #if they killed all the deer, then go to millet if possible
                    print "Killed all the deer available this year, will try to make up the remainder of the diet with millet"
                    deerscore = 0.
                    pass
                else:
                ...
                if gtimebudget <= 0:
                    print "Ran out of gathering time this year, hopefully there is hunting time left"
                    milletscore = 0
                    pass
                elif gtimebudget <= 0 and htimebudget > 0:
                    print "Using remaining hunting time to gather millet"
                    ...
                    print "Not enough hunting time left to use for gathering"
                    milletscore = 0
                    pass
                else: pass
                if millet_now <= 0: #if millet is all gone, then go back to deer
                    print "Harvested all available millet this year, will try to make up the remainder of the diet with deer."
                    milletscore = 0
                    pass
                else:
                    kcalneed = kcalneed - (mret * mpatch_now)
            ...
            if htimebudget <= 0 and gtimebudget <= 0:        #check if they've run out of foraging time, and stop the loop if necessary.
                print "Ran out of all foraging time for this year before gathering enough food."
                break
            if deer <= 0 and millet <= 0:    #check if they've run out of food, and stop the loop if necessary.
                print "Ate all the deer and all the millet this year before gathering enough food."
                break
        ...
        if (people * hkcal) - kcalneed <= (people * hkcal * starvthresh):     #Check if they starved this year and just die deaths if so
            print "Starved a bit this year, no births will occur."
        ...
        #This part is a bit complicated. We are adjusting the proportions of wild to domestic millet in JUST the millet patches that were exploited this year. We are also adjusting the density of individuals in those patches. This is the effect of the "artifical selection" exhibited by humans while exploiting those patches. At the same time, we are implementing a "diffusion" of wild-type characteristics back to these patches if they aren't being used. This is a bit of simplification because diffusion is probably also happening if they ARE being used, but it just gets swamped out by the larger pressure exerted by artificial selection. It is programmatically simpler, however, to have diffusion operate only if a patch was not used. We make up for this simplification by "tuning" the rate of artificial selection to account for the unequal, but opposite counter-effect of concurrent diffusion of wild-type traits. So it's really an "effective" selection rate.
        ...
        for x in range(1,int(millet+1)):
            if x < eatmillet:
                patch_adjust.append([-mselect, cultiv])
            else:
                patch_adjust.append([mdiffus, -cultiv])
        ...
        millet_df['mprop'] = millet_df['mprop'].where((millet_df['mprop'] + patch_adjustdf['sel'] > 1 - mdiffus) | (millet_df['mprop'] + patch_adjustdf['sel'] < 0 + mselect), other=millet_df['mprop'] + patch_adjustdf['sel']) # adjust the selection coefficient column, but only if the value will stay between 1 and 0.

        millet_df.to_csv('supposed_new_patches.csv')
    ...
    if "Plot" in choices:
        ...
        if pf is '':
            ...
            plt.savefig(pf, format = pf.split('.')[1], frameon = True)
    else:
        pass
